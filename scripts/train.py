...
MODELS_CONFIG = {
    "SUDENAZ": ["tf_efficientnetv2_b0"], # Hafif modelle başlıyoruz
...
def run_fair_training_pipeline(model_name, train_loader, val_loader):
    ...
    print(f"⚖️ ŞARTLAR: LR={STANDARD_LR}, WD={STANDARD_WD}, Epoch={STANDARD_EPOCHS}")
    ...
    for epoch in range(STANDARD_EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({'model_state_dict': model.state_dict(), 'acc': val_acc}, save_path)
...
if __name__ == "__main__":
    ...
    for m_name in modellerim:
        run_fair_training_pipeline(m_name, train_loader, val_loader)
